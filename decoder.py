...
def decode_name(bytes_seq: list[int]) -> str:
    """Stop at 0x50 (<END>) instead of printing it."""
    out = []
    for b in bytes_seq:
        if b == 0x50:
            break
...
# Load raw dump: list of [addr_hex, label, raw_value]
...
for addr_hex, label, raw in dump:
    ...
    # only include "decoded" if it differs from raw
    if human is not None and human != raw:
        entry["decoded"] = human
